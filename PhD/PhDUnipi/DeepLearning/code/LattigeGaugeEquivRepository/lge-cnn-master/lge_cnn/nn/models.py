...
"""
    LGE-CNN Model definition
"""


class GCMConvNet(pl.core.LightningModule):
    def __init__(self, hparams):
        super(GCMConvNet, self).__init__()
        ...
        for i in range(len(conv_ch) - 1):
            if use_symmetric:
                conv = GCMConvSymmetric(dims=hparams.dims,
                ...
                conv = GCMConv(dims=hparams.dims,
                               kernel_size=hparams.conv_kernel_size[i],
                               dilation=hparams.conv_dilation[i],
                               n_in=conv_ch[i],
                               n_out=conv_ch[i + 1],
    ...
    def update_dims(self, new_dims):
        if len(new_dims) != len(self.hparams.dims):
            raise ValueError("Can not change lattice dimensions, only lattice size!")
