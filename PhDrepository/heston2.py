...
def generate_heston_paths(
    S, T, r, kappa, theta, v_0, rho, xi, steps, Npaths, return_vol=False
):
    dt = T / steps
    size = (Npaths, steps)
    ...
    for t in range(steps):
        WT = np.random.multivariate_normal(
            np.array([0, 0]), cov=np.array([[1, rho], [rho, 1]]), size=paths
        ) * np.sqrt(dt)
...
prices_pos = generate_heston_paths(
    S,
    T,
    r,
    kappa,
...
prices_neg = generate_heston_paths(
    S,
    T,
    r,
    kappa,
...
prices_zero = generate_heston_paths(
    S,
    T,
    r,
    kappa,
...
gbm_bench = S * np.exp(
    np.random.normal((r - v_0 / 2) * T, np.sqrt(theta) * np.sqrt(T), size=paths)
)
...
T = 1
rho = -0.8
prices, sigs = generate_heston_paths(
    S, T, r, kappa, theta, v_0, rho, xi, steps, paths, return_vol=True
)
